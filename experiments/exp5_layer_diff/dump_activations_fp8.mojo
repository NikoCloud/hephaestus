...
def forward_dump[
    ...
    for i in range(n_layers):
        ...
        linear_fp8(
            q, xn, layer.q_proj, layer.q_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, q_out, hidden, ctx
        )
        if do_dump:
            dump_bf16_device(
                dump_dir,
        ...
        linear_fp8(
            k_dst, xn, layer.k_proj, layer.k_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, kv_out, hidden, ctx,
            do_quantize=False,
        )
        if do_dump:
            dump_bf16_device(
        ...
        linear_fp8(
            v_dst, xn, layer.v_proj, layer.v_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, kv_out, hidden, ctx,
            do_quantize=False,
        )
        if do_dump:
        ...
        linear_add_residual_fp8(
            x, attn_out, layer.o_proj, layer.o_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, hidden, q_out, ctx
        )
        if do_dump:
            dump_bf16_device(
                dump_dir,
        ...
        linear_fp8(
            gate, xn, layer.gate_proj, layer.gate_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, inter, hidden, ctx
        )
        if do_dump:
            dump_bf16_device(
                dump_dir,
        ...
        linear_fp8(
            up, xn, layer.up_proj, layer.up_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, inter, hidden, ctx,
            do_quantize=False,
        )
        if do_dump:
        ...
        linear_add_residual_fp8(
            x, act, layer.down_proj, layer.down_proj_scale,
            acts.a_fp8_pad, acts.act_scale, seq, hidden, inter, ctx
        )
        if do_dump:
            dump_bf16_device(
                dump_dir,
    ...
    linear_fp8[F32](
        logits, xn, weights.embed_tokens, weights.embed_scale,
        acts.a_fp8_pad, acts.act_scale, seq, vocab, hidden, ctx
    )
    dump_f32_device(
        dump_dir,
        fprefix + "lm_head",
...
def run_tiny_fp8(dump_dir: String, prompt_idx: Int) raises:
    var ids = prompt_ids_tiny(prompt_idx)
    var seq = len(ids)
    print("dump tiny FP8 W8A8 prompt", prompt_idx, "seq", seq, "->", dump_dir)

    var ctx = DeviceContext()
    var arena = load_arena_bytes(ctx, "staged/tiny-fp8")
    var base_ptr = (
    ...
    ctx.synchronize()
    print("tiny FP8 dump complete")


def run_4b_removed() raises:
    raise Error("use dump_activations for 4b BF16")



def main() raises:
    if len(argv()) < 3:
        print("usage: dump_activations_fp8 tiny <dump_dir> [prompt_idx]")
    ...
    if mode == "tiny":
        ...
        run_tiny_fp8(dump_dir, prompt_idx)
    else:
        raise Error("mode must be tiny")
